...
- listar los SitReps de ReliefWeb y extraer su texto → docs/tareas/sitreps-ve.json + docs/tareas/sitreps/*.txt
- descargar de PRODUCCIÓN el index.html vigente y dejarlo en ./index.html (tu base de trabajo)
...
- NO hagas git commit, git push, git checkout ni git stash. NO toques ningún archivo que no sea index.html (ni noticias*.json ni colombia.html ni docs/tareas/).
...
Los textos de cada SitRep están en docs/tareas/sitreps/*.txt (campo texto de cada reporte).
...
- HTML entero: contiene noticias.json, <title y </html>; secciones abiertas == cerradas (python3 -c "import re;s=open('index.html').read();print(len(re.findall(r'<section',s)),len(re.findall(r'</section>',s)))"); arrays ORGANISMOS y ORG_RUBROS con la misma longitud.
    git status --porcelain     # SOLO " M index.html" (o nada)

=========================================================
7. AL TERMINAR
=========================================================
No commitees ni pushees: lo hace el workflow. Di qué SitRep había, cuál(es) integraste, qué cifras cambiaron (con corte y fuente), qué quedó igual y por qué. Si no hubo novedad, una línea.
